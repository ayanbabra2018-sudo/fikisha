...
def save_db(db):
    with DB_LOCK:
        ...
        try:
            ...
            print(f"Local save failed: {e}"); return False
...
def delete_school(code):
    try:
        db = load_db(); db['schools'].pop(normalize_code(code), None); save_db(db)
    except:
        pass
    return redirect("/")
...
def delete_van_route(code, plate):
    try:
        ...
        if school and not is_locked(school):
            ...
            for kid_id in [kid_id for kid_id, kid in (school.get('kids',{}) or {}).items() if (kid or {}).get('van_plate')==p]:
                school['kids'].pop(kid_id, None)
            save_db(db)
    except:
        pass
    return redirect(f"/admin/{normalize_code(code)}")
...
def driver_action(code, plate):
    try:
        ...
        school['attendance_log']=log[-200:]; save_db(db); return jsonify({"ok": True})
    except Exception as e:
        print(f"action crash {e}"); return jsonify({"ok": False, "error": str(e)}), 500
...
def mass_action(code, plate):
    try:
        ...
        if act == 'dropped_school':
            ...
            school['attendance_log']=log[-200:]; save_db(db); return jsonify({"ok": True, "count": count, "skipped": count==0})
        return jsonify({"ok": False}), 400
    except Exception as e:
...
def traffic(code, plate):
    try:
        ...
        try:
            last_traffic = school.get('last_traffic', {}) or {}
            last_ts_str = last_traffic.get(plate_norm)
            if last_ts_str:
                last_ts = datetime.fromisoformat(last_ts_str)
                now_cmp = kampala_now()
                if last_ts.tzinfo is None:
                    now_cmp = now_cmp.replace(tzinfo=None) if now_cmp.tzinfo else now_cmp
                else:
                    if now_cmp.tzinfo is None and last_ts.tzinfo:
                        last_ts = last_ts.replace(tzinfo=None)
                if (now_cmp - last_ts).total_seconds() < 300:
                    remaining = int(300 - (now_cmp - last_ts).total_seconds())
                    return jsonify({"ok": False, "error": f"Wait {remaining}s - traffic alert already sent", "cooldown": True}), 429
        except:
            pass
        clean_msg = raw_msg[:100].strip(); sent = 0
        for kid in [k for k in (school.get('kids',{}) or {}).values() if k and k.get('van_plate','')==plate_norm and not k.get('absent')]:
            whatsapp_async(kid.get('parent_phone',''), "traffic_alert", [plate_norm, clean_msg]); sent+=1
        if 'last_traffic' not in school: school['last_traffic'] = {}
        school['last_traffic'][plate_norm] = kampala_now().isoformat()
        save_db(db)
...
def bulk_sync(code, plate):
    try:
        ...
        short = kampala_now().strftime("%I:%M %p"); full = kampala_now().strftime("%I:%M %p %d %b"); count=0; skipped=0; log=school.get('attendance_log',[]) or []
        for it in items:
            ...
            if act == 'absent' and kid.get('absent'):
                skipped+=1; continue
            if act=='picked_home':
                kid['status']=f"On way to School - picked at {short}"; kid['times']['picked_home']=full; kid['absent']=False; whatsapp_async(kid.get('parent_phone',''), "picked_home", [kid.get('name',''), short, plate_norm]); count+=1; log.append({"date": str(date.today()), "kid": kid.get('name',''), "action": "picked_home", "time": full, "van": plate_norm})
            elif act=='dropped_school':
                kid['status']=f"At School - arrived at {short}"; kid['times']['dropped_school']=full; whatsapp_async(kid.get('parent_phone',''), "dropped_school", [kid.get('name',''), short]); count+=1; log.append({"date": str(date.today()), "kid": kid.get('name',''), "action": "dropped_school", "time": full, "van": plate_norm})
            elif act=='picked_school':
                kid['status']=f"On way Home - left at {short}"; kid['times']['picked_school']=full; whatsapp_async(kid.get('parent_phone',''), "picked_school", [kid.get('name',''), short, plate_norm]); count+=1; log.append({"date": str(date.today()), "kid": kid.get('name',''), "action": "picked_school", "time": full, "van": plate_norm})
            elif act=='dropped_home':
                kid['status']=f"Home Safe - {short}"; kid['times']['dropped_home']=full; kid['dropped_home_ts']=kampala_now().isoformat(); whatsapp_async(kid.get('parent_phone',''), "dropped_home", [kid.get('name',''), short]); count+=1; log.append({"date": str(date.today()), "kid": kid.get('name',''), "action": "dropped_home", "time": full, "van": plate_norm})
            elif act=='absent':
                kid['status']="ABSENT Today"; kid['absent']=True; whatsapp_async(kid.get('parent_phone',''), "absent", [kid.get('name',''), str(date.today()), plate_norm]); count+=1; log.append({"date": str(date.today()), "kid": kid.get('name',''), "action": "ABSENT", "time": full, "van": plate_norm})
            elif act=='present':
                kid['absent']=False; kid['status']="At Home - waiting for van"; kid['times']={}; kid.pop('dropped_home_ts',None); count+=1; log.append({"date": str(date.today()), "kid": kid.get('name',''), "action": "PRESENT", "time": full, "van": plate_norm})
        school['attendance_log']=log[-200:]; save_db(db); return jsonify({"ok": True, "count": count, "skipped": skipped})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def weekly_report(code):
    try:
        ...
        if log:
            ...
            for e in log:
                html += f"<tr><td>{e.get('date','')}</td><td>{e.get('kid','')}</td><td>{e.get('van','')}</td><td>{e.get('action','')}</td><td><small>{e.get('time','')}</small></td></tr>"
            html += "</table></div>"
        return html
    except Exception as e:
...
def export_csv_weekly(code):
    try:
        ...
        for e in (school.get('attendance_log',[]) or []):
...
def health():
    try:
        return jsonify({"ok": True, "schools": len((load_db().get("schools", {}) or {})), "supabase": bool(SUPABASE_URL)})
    except:
