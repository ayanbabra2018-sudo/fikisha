...
def mass_action(code, plate):
    try:
        ...
        if act == 'dropped_school':
            ...
            school['attendance_log']=log[-200:]; save_db(db); return jsonify({"ok": True, "count": count})
        return jsonify({"ok": False}), 400
    except Exception as e:
