...
def parent_view(kid_id):
    ...
    return "Kid not found - check link"

@app.route("/report/<code>")
def report(code):
    db = load_db(); code = normalize_code(code); school = db['schools'].get(code)
    if not school: return "No school"
    html = CSS + f"<h2>Daily Report {school['name']} - {date.today()} - {current_time_str()}</h2><a href='/admin/{code}'>Back Admin</a> <button onclick='window.print()' class='no-print'>Print</button><table><tr><th>Kid</th><th>Van</th><th>Status</th><th>Picked Home</th><th>Dropped School</th><th>Picked School</th><th>Dropped Home</th></tr>"
    for kid in school['kids'].values():
        html += f"<tr><td>{kid['name']}</td><td>{kid['van_plate']}</td><td>{kid['status']}</td><td>{kid['times'].get('picked_home','')}</td><td>{kid['times'].get('dropped_school','')}</td><td>{kid['times'].get('picked_school','')}</td><td>{kid['times'].get('dropped_home','')}</td></tr>"
    html += "</table>"
    return html
